...
def train_and_save_model():
    ...
    numeric_cols = ['followers_count','following_count','tweet_count',
                    'listed_count','account_age_days']
    df[numeric_cols] = df[numeric_cols].fillna(0)

    df['follow_ratio']     = df['followers_count'] / (df['following_count'] + 1)
    df['age_score']        = 1 / (df['account_age_days'] + 1)
    df['engagement_ratio'] = (df['followers_count'] + df['listed_count']) / (df['tweet_count'] + 1)
    ...
    df['low_followers']    = (df['followers_count'] < 50).astype(int)
    df['tweet_count_log']  = df['tweet_count'].astype(float).apply(np.log1p)
    df['listed_count_log'] = df['listed_count'].astype(float).apply(np.log1p)
...
def compute_features(account):
    followers = float(account.get('followers_count') or 0)
    following = float(account.get('following_count') or 0)
    tweets    = float(account.get('tweet_count') or 0)
    listed    = float(account.get('listed_count') or 0)
    age_days  = float(account.get('account_age_days') or 0)
